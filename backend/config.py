...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "brAIn")
if WEBUI_NAME != "brAIn":
    WEBUI_NAME += " (brAIn)"

WEBUI_URL = os.environ.get("WEBUI_URL", "http://localhost:3000")

WEBUI_FAVICON_URL = "https://brAIn.com/favicon.png"
...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.brAIn.com/api/v1/custom/{CUSTOM_NAME}")
        data = r.json()
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = (
                    f"https://api.brAIn.com{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (brAIn + Ollama)
